...
from tests.data.aibom.aibom_sample import CONTAINER_ON_CHILD_IMAGE
...
def test_runs_on_analysis_same_manifest_list(
    mock_json_files,
    mock_file_open,
    neo4j_session,
):
    """RUNS_ON is created when AIBOMSource and Container both point to the same ImageManifestList."""
    _seed_aibom_with_containers(
        neo4j_session,
        AIBOM_REPORT,
...
def test_runs_on_analysis_manifest_list_child_hop(
    mock_json_files,
    mock_file_open,
    neo4j_session,
):
    """RUNS_ON is created when AIBOMSource scans a manifest list and Container has a child image."""
    _seed_aibom_with_containers(
        neo4j_session,
        AIBOM_REPORT,
        _seed_manifest_list_graph,
        [CONTAINER_ON_CHILD_IMAGE],
    )
    ...
    assert check_rels(
        ...
        "RUNS_ON",
        rel_direction_right=True,
    ) == {(TEST_SOURCE_KEY, CONTAINER_ON_CHILD_IMAGE["uid"])}
