...
def test_sync_aks(mock_get_clusters, mock_get_pools, neo4j_session):
    ...
    # Assert Agent Pools (the same mocked pool is attached to both clusters in this fixture).
...
def test_load_aks_tags(neo4j_session):
    ...
    # Assert: Check the relationships (both fixture clusters carry the same tags).
    expected_rels = {
        (MOCK_CLUSTERS[0]["id"], f"{TEST_SUBSCRIPTION_ID}|env:prod"),
        (MOCK_CLUSTERS[0]["id"], f"{TEST_SUBSCRIPTION_ID}|service:aks"),
        (MOCK_CLUSTERS[1]["id"], f"{TEST_SUBSCRIPTION_ID}|env:prod"),
        (MOCK_CLUSTERS[1]["id"], f"{TEST_SUBSCRIPTION_ID}|service:aks"),
    }
    actual_rels = check_rels(
        neo4j_session,
        "AzureKubernetesCluster",
