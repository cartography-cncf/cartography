...
async def test_sync_o365_licenses(mock_get_users, neo4j_session):
    ...
    expected_service_plans = {
        (
            str(MOCK_SUBSCRIBED_SKUS[0].service_plans[0].service_plan_id),
            "EXCHANGE_S_ENTERPRISE",
        ),
        (
            str(MOCK_SUBSCRIBED_SKUS[0].service_plans[1].service_plan_id),
    ...
    expected_has_sp_rels = {
        ...
        (
            "tenant-sku-e5",
            str(MOCK_SUBSCRIBED_SKUS[0].service_plans[2].service_plan_id),
        ),
        (
            "tenant-sku-ems",
            str(MOCK_SUBSCRIBED_SKUS[1].service_plans[0].service_plan_id),
        ),
        (
            "tenant-sku-ems",
            str(MOCK_SUBSCRIBED_SKUS[0].service_plans[0].service_plan_id),
