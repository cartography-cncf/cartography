...
import tests.data.circleci.schedules
...
@patch.object(
    cartography.intel.circleci.schedules,
    "get",
    return_value=tests.data.circleci.schedules.CIRCLECI_SCHEDULES,
)
def test_load_circleci_schedules(mock_api, neo4j_session):
    # Arrange
    ...
    # Assert schedules exist with their actor
    assert check_nodes(neo4j_session, "CircleCISchedule", ["id", "actor_login"]) == {
        ("sched-1", "alice"),
    }

    # Assert (Project)-[:RESOURCE]->(Schedule)
    assert check_rels(
        neo4j_session,
        "CircleCISchedule",
        "id",
