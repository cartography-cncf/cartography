...
def _repos_without_privileged_fields():
    repos = deepcopy(GET_REPOS)
    for repo in repos:
...
def _all_privileged_repo_details():
    return {
        GET_REPOS[0]["url"]: {
        ...
        GET_REPOS[2]["url"]: {
            ...
            "branchProtectionRules": GET_REPOS[2]["branchProtectionRules"],
        },
    }


def _partial_privileged_repo_details():
    return {
        GET_REPOS[2]["url"]: {
            "directCollaborators": {"totalCount": 3},
            "outsideCollaborators": {"totalCount": 5},
...
def _collabs_side_effect_from_repo_counts(
    repo_raw_data, affiliation, org, api_url, token
):
    collab_counts_key = "directCollaborators"
    collab_data = DIRECT_COLLABORATORS
    if affiliation == "OUTSIDE":
        collab_counts_key = "outsideCollaborators"
        collab_data = OUTSIDE_COLLABORATORS
    ...
    for repo in repo_raw_data:
        if repo is None:
            continue
        repo_url = repo["url"]
        repo_collab_counts = repo.get(collab_counts_key) or {}
        collab_count = repo_collab_counts.get("totalCount", 0)
        result[repo_url] = collab_data.get(repo_url, []) if collab_count > 0 else []
    return result
...
@patch.object(cartography.intel.github.repos, "get_repo_privileged_details_by_url")
def test_sync_github_repos_with_privileged_merge(
    mock_get_privileged_details,
    mock_get_collabs,
    mock_get_repos,
    neo4j_session,
):
    local_job_params = {"UPDATE_TAG": TEST_UPDATE_TAG + 1000}
    mock_get_repos.return_value = _repos_without_privileged_fields()
    mock_get_privileged_details.return_value = _all_privileged_repo_details()
    mock_get_collabs.side_effect = _collabs_side_effect_from_repo_counts
    ...
    actual_repo_ids = {
        repo_id[0] for repo_id in check_nodes(neo4j_session, "GitHubRepository", ["id"])
    }
    assert expected_repo_ids.issubset(actual_repo_ids)

    actual_direct_collab_admin_rels = check_rels(
        neo4j_session,
        "GitHubUser",
        "username",
        "GitHubRepository",
    ...
    filtered_direct_collab_admin_rels = {
        rel
        for rel in actual_direct_collab_admin_rels
        if rel[1] in {"SampleRepo2", "cartography"}
    }
    assert filtered_direct_collab_admin_rels == {
        ("direct_foo", "SampleRepo2"),
        ("SecPrez", "cartography"),
    }

    actual_has_rule_rels = check_rels(
        neo4j_session,
        "GitHubRepository",
        "id",
        "GitHubBranchProtectionRule",
    ...
    filtered_has_rule_rels = {
        rel
        for rel in actual_has_rule_rels
        if rel[0] == "https://github.com/cartography-cncf/cartography"
    }
    assert filtered_has_rule_rels == {
        (
            "https://github.com/cartography-cncf/cartography",
            "BPR_kwDOAbc123==",
        ),
...
@patch.object(cartography.intel.github.repos, "get_repo_privileged_details_by_url")
def test_sync_github_repos_with_partial_privileged_details(
    mock_get_privileged_details,
    mock_get_collabs,
    mock_get_repos,
    neo4j_session,
):
    local_job_params = {"UPDATE_TAG": TEST_UPDATE_TAG + 1001}
    mock_get_repos.return_value = _repos_without_privileged_fields()
    mock_get_privileged_details.return_value = _partial_privileged_repo_details()
    mock_get_collabs.side_effect = _collabs_side_effect_from_repo_counts
    ...
    # Only the repo that got privileged collaborator counts should be enriched.
    actual_direct_collab_admin_rels = check_rels(
        neo4j_session,
        "GitHubUser",
        "username",
    ...
    # Branch protection enrichment should also be partial and only for repos with details.
    actual_has_rule_rels = check_rels(
        neo4j_session,
        "GitHubRepository",
        "id",
