...
def test_sync_cloudwatch_log_metric_filters(mock_get_metric_filters, mock_get_log_groups, neo4j_session):
    # Arrange
    boto3_session = MagicMock()
    create_test_account(neo4j_session, TEST_ACCOUNT_ID, TEST_UPDATE_TAG)
    ...
    # Assert Log Metric Filters were added as nodes
    assert check_nodes(neo4j_session, "CloudWatchLogMetricFilter", ["filterName"]) == {
        ("LambdaErrorCount",),
        ("BuildDuration",),
    }

    # Assert Log Metric Filters are connected to their Log Groups
    assert check_rels(
        neo4j_session,
        "CloudWatchLogGroup",
        "arn",
