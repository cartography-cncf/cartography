...
- Integration tests expect a local Neo4j instance, defaulting to
  `bolt://localhost:7687` with auth disabled unless `NEO4J_URL` is set.
- The integration test suite deletes graph data as part of its setup; use a
  disposable local Neo4j database.
- Use focused pytest commands while iterating, for example:
