...
def test_finding_output_display_name_fields():
    for finding_id, finding in FINDINGS.items():
        ...
        display_name_field = finding.output_model.model_fields.get(
            "display_name_fields"
        )
        assert (
            display_name_field is not None
        ), f"FindingOutput model '{finding.output_model.__name__}' for finding '{finding_id}' must have a display_name_fields field."

        display_name_fields = display_name_field.default
        assert (
            display_name_fields and display_name_fields[-1] == "id"
        ), f"You must include 'id' as the ultimate fallback in the display_name_fields of FindingOutput model '{finding.output_model.__name__}' for finding '{finding_id}'. Current display_name_fields: {display_name_fields}"
