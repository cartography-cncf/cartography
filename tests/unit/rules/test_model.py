...
def test_all_module_keys_in_mapping_except_cross_cloud():
    ...
    assert missing_modules == set(), f"The following modules are missing from MODULE_TO_CARTOGRAPHY_INTEL: {missing_modules}"

    # Assert no extra modules are in the mapping
    assert extra_modules == set(), f"The following unexpected modules are in MODULE_TO_CARTOGRAPHY_INTEL: {extra_modules}"
...
def test_cross_cloud_not_in_mapping():
    ...
    """
    assert Module.CROSS_CLOUD not in MODULE_TO_CARTOGRAPHY_INTEL, (
...
def test_mapping_values_exists():
    ...
    for module, intel_name in MODULE_TO_CARTOGRAPHY_INTEL.items():
        ...
        assert intel_name in TOP_LEVEL_MODULES, (
