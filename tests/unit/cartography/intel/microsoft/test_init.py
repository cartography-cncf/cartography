...
@patch("cartography.intel.microsoft.start_intune_ingestion")
@patch("cartography.intel.microsoft.start_entra_ingestion")
def test_delegated_auth_logs_skipped_modules_before_entra_failure(
    mock_start_entra,
    mock_start_intune,
    mock_start_o365,
    caplog,
    ...
    mock_start_entra.side_effect = RuntimeError("partial delegated sync")
    ...
    with (
        ...
        pytest.raises(
            RuntimeError,
            match="partial delegated sync",
        ),
    ):
        start_microsoft_ingestion(MagicMock(), config)

    assert "Intune and O365 ingestion were not attempted" in caplog.text
    mock_start_intune.assert_not_called()
    mock_start_o365.assert_not_called()
