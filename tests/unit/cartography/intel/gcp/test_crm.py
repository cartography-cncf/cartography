...
def test_get_gcp_projects_success():
    ...
    # Mock second page (empty - end of pagination)
    mock_request2 = Mock()
...
def test_load_gcp_organizations():
    ...
    # Verify the correct Cypher query was called
    expected_calls = []
    for org in GCP_ORGANIZATIONS:
        expected_calls.append(call(
            ANY,  # The query string
            OrgName=org["name"],
...
def test_load_gcp_folders_with_organization_parent():
    ...
    # Should be called once for each folder
...
def test_load_gcp_projects():
    ...
    with patch.object(crm, '_attach_gcp_project_parent') as mock_attach:
        ...
        # Should call run for each project
        assert mock_neo4j_session.run.call_count == len(GCP_PROJECTS)
        
        # Should call _attach_gcp_project_parent for projects with parents
...
def test_load_gcp_projects_without_parent():
    ...
    with patch.object(crm, '_attach_gcp_project_parent') as mock_attach:
        ...
        # Should not call _attach_gcp_project_parent for projects without parents
...
def test_attach_gcp_project_parent_folder():
    ...
    project_with_folder_parent = GCP_PROJECTS[0]  # Has folder parent
