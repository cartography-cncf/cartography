...
def test_parse_okta_group_name() -> None:
    ...
    # Act
    account_role: Optional[AccountRole] = _parse_okta_group_name(
        group_name,
        mapping_regex,
    )
...
def test_query_for_okta_to_awssso_role_mapping(
    mock_get_awssso_role_arn: MagicMock,
) -> None:
    # Arrange
    ...
    # Act
    result = query_for_okta_to_awssso_role_mapping(
        neo4j_session,
        okta_groups,
        mapping_regex,
    )
    ...
    assert result == [
        GroupRole(
            okta_group_id="0oaxm1",
            aws_role_arn="arn:aws:iam:1234:role/AWSReservedSSO_myrole1_abcdef",
        ),
        GroupRole(
            okta_group_id="0oaxm2",
            aws_role_arn="arn:aws:iam:1234:role/AWSReservedSSO_myrole2_bcdefa",
        ),
        GroupRole(
            okta_group_id="0oaxm3",
            aws_role_arn="arn:aws:iam:1234:role/AWSReservedSSO_myrole3_cdefab",
        ),
    ]
