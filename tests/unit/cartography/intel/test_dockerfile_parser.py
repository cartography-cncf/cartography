...
class TestNormalizeCommand:
    ...
    def test_normalize_removes_shell_prefix(self):
        cmd = "/bin/sh -c pip install flask"
        assert normalize_command(cmd) == "pip install flask"

    def test_normalize_removes_instruction_prefix(self):
    ...
    def test_normalize_removes_buildkit_prefix(self):
        cmd = "|1 VAR=value pip install flask"
        assert normalize_command(cmd) == "pip install flask"

    def test_normalize_removes_buildkit_mount(self):
        cmd = "--mount=type=cache,target=/root/.cache pip install flask"
        assert normalize_command(cmd) == "pip install flask"

    def test_normalize_removes_nop_marker(self):
        cmd = "#(nop) WORKDIR /app"
...
"""
    ...
    def test_parse_multistage(self):
        content = """
...
"""
    ...
    def test_parse_extracts_base_image_info(self):
...
class TestExtractLayerCommands:
    """Tests for extracting layer commands from image history."""
    ...
    def test_extract_last_n_commands(self):
        history = [
...
class TestCommandSimilarity:
    """Tests for command similarity computation."""

    def test_identical_commands(self):
        assert (
            compute_command_similarity("pip install flask", "pip install flask") == 1.0
        )
...
class TestFindBestMatches:
    """Tests for finding best Dockerfile matches."""
    ...
    def test_find_best_among_multiple(self):
