...
def test_search_fallback_used(monkeypatch):
    ...
    # Apply monkeypatches
    monkeypatch.setattr(
        tgw_route_tables, "get_transit_gateway_route_tables", fake_get_rts
    )
    monkeypatch.setattr(
        tgw_route_tables, "get_transit_gateway_routes_for_table", fake_search_routes
    )
    monkeypatch.setattr(
        tgw_route_tables, "load_transit_gateway_routes", fake_load_routes
    )
    monkeypatch.setattr(
        tgw_route_tables, "load_transit_gateway_route_tables", fake_load_rtbs
    )
    # Associations/propagations are fetched via real boto3 clients; stub them out
    # so this test does not attempt live AWS calls.
    monkeypatch.setattr(
        tgw_route_tables,
