...
## Cross-Product MatchLinks

Use `load_matchlinks_cross_product()` only when every source node should be
...
Cross-product MatchLinks use the same `CartographyRelSchema` shape and the same
