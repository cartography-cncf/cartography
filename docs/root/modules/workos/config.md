...
1. Prepare your WorkOS API credentials.
    ...
    1. Populate environment variables with the API Key and Client ID. Pass the API key environment variable name via the `--workos-apikey-env-var` CLI arg, and the Client ID via the `--workos-client-id` CLI arg.
