...
| exposed\_internet\_type | A list indicating the type(s) of internet exposure. Set by the `AWS_EC2_ASSET_EXPOSURE_JOBS` analysis jobs in [cartography/models/aws/analysis.py](https://github.com/cartography-cncf/cartography/blob/master/cartography/models/aws/analysis.py). |
| **id** |  Currently set to the `dnsname` of the load balancer. |
...
| exposed\_internet | Set to `True` if any EC2 instance in this Auto Scaling Group is exposed to the internet. Set by the `AWS_EC2_ASSET_EXPOSURE_JOBS` analysis jobs in [cartography/models/aws/analysis.py](https://github.com/cartography-cncf/cartography/blob/master/cartography/models/aws/analysis.py). |
...
| exposed\_internet | Set to `True` if this container is exposed to the internet via an internet-facing load balancer. Set by the `AWS_ECS_ASSET_EXPOSURE` analysis job in [cartography/models/aws/analysis.py](https://github.com/cartography-cncf/cartography/blob/master/cartography/models/aws/analysis.py). |
