...
- SpaceliftAccount contains all other Spacelift resources via RESOURCE relationships:
    ...
    (SpaceliftAccount)-[RESOURCE]->(GitCommit)
    ```
...
- SpaceliftRuns can affect EC2 Instances:
    ...
    ```
    (SpaceliftRun)-[AFFECTS{action}]->(EC2Instance)
    ```
...
- GitCommits belong to a SpaceliftAccount:

    ```
    (GitCommit)<-[RESOURCE]-(SpaceliftAccount)
    ```

- GitCommits are confirmed by SpaceliftUsers:
