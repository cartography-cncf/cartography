...
| `FileStorage` | `> **Ontology Mapping**: This node has the extra label \`FileStorage\` to enable cross-platform queries for network file systems and shares across different systems (e.g., EfsFileSystem, AzureDataLakeFileSystem, AzureStorageFileShare).` |
