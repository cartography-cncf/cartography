...
class AppContext:
    def __init__(
        ...
        self.aws_iam_compliance_min_days_for_rotation = (
            aws_iam_compliance_min_days_for_rotation
        )
        self.aws_iam_compliance_min_days_for_update = (
            aws_iam_compliance_min_days_for_update
        )
        ...
        self.aws_ec2_monitoring_outdated_instance_types = (
            aws_ec2_monitoring_outdated_instance_types
        )
        ...
        self.aws_dynamodb_monitoring_request_topic = (
            aws_dynamodb_monitoring_request_topic
        )
        self.aws_dynamodb_monitoring_response_topic = (
            aws_dynamodb_monitoring_response_topic
        )
        ...
        self.aws_kubernetes_monitoring_request_topic = (
            aws_kubernetes_monitoring_request_topic
        )
        self.aws_kubernetes_monitoring_response_topic = (
            aws_kubernetes_monitoring_response_topic
        )
        self.aws_kubernetes_monitoring_latest_versions = (
            aws_kubernetes_monitoring_latest_versions
        )

        self.aws_apigateway_monitoring_request_topic = (
            aws_apigateway_monitoring_request_topic
        )
        self.aws_apigateway_monitoring_response_topic = (
            aws_apigateway_monitoring_response_topic
        )
        self.aws_cloudfront_monitoring_request_topic = (
            aws_cloudfront_monitoring_request_topic
        )
        self.aws_cloudfront_monitoring_response_topic = (
            aws_cloudfront_monitoring_response_topic
        )
        self.aws_cloudtrail_monitoring_request_topic = (
            aws_cloudtrail_monitoring_request_topic
        )
        self.aws_cloudtrail_monitoring_response_topic = (
            aws_cloudtrail_monitoring_response_topic
        )
        self.aws_redshift_monitoring_request_topic = (
            aws_redshift_monitoring_request_topic
        )
        self.aws_redshift_monitoring_response_topic = (
            aws_redshift_monitoring_response_topic
        )
        self.aws_redshift_monitoring_idleconnection_limit_per_day = (
            aws_redshift_monitoring_idleconnection_limit_per_day
        )
        self.aws_redshift_monitoring_idle_readiops_limit_per_day = (
            aws_redshift_monitoring_idle_readiops_limit_per_day
        )
        self.aws_redshift_monitoring_idle_writeiops_limit_per_day = (
            aws_redshift_monitoring_idle_writeiops_limit_per_day
        )
        self.aws_redshift_monitoring_maximum_diskspace_usage_limit = (
            aws_redshift_monitoring_maximum_diskspace_usage_limit
        )
        self.aws_redshift_monitoring_underutilized_average_cpu_usage_limit = (
            aws_redshift_monitoring_underutilized_average_cpu_usage_limit
        )
        self.aws_redshift_monitoring_underutilized_readiops_limit_per_day = (
            aws_redshift_monitoring_underutilized_readiops_limit_per_day
        )
        self.aws_redshift_monitoring_underutilized_writeiops_limit_per_day = (
            aws_redshift_monitoring_underutilized_writeiops_limit_per_day
        )
        self.aws_cloudformation_monitoring_request_topic = (
            aws_cloudformation_monitoring_request_topic
        )
        self.aws_cloudformation_monitoring_response_topic = (
            aws_cloudformation_monitoring_response_topic
        )
        ...
        self.aws_cloudwatch_monitoring_request_topic = (
            aws_cloudwatch_monitoring_request_topic
        )
        self.aws_cloudwatch_monitoring_response_topic = (
            aws_cloudwatch_monitoring_response_topic
        )
        self.aws_route53_monitoring_request_topic = aws_route53_monitoring_request_topic
        self.aws_route53_monitoring_response_topic = (
            aws_route53_monitoring_response_topic
        )
        ...
        self.aws_remediation_kms_datakey_reuse_period_seconds = (
            aws_remediation_kms_datakey_reuse_period_seconds
        )
        ...
        self.aws_cost_anomaly_total_days_to_prefill = (
            aws_cost_anomaly_total_days_to_prefill
        )
    ...
    def parse(self, config):
        ...
        self.aws_excluded_regions = (
            self.get_value(config, ["common", "aws", "excludedRegions"]) or []
        )
        ...
        if config.get("audit"):
            ...
            self.aws_iam_compliance_request_topic = self.get_sns_topic(
                ...
                self.audit_sns_request_topic,
            )
            self.aws_iam_compliance_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["iamcompliance", "resTopic"]),
            ...
            self.aws_sg_audit_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["sgaudit", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_s3_public_check_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["s3publiccheck", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_s3_public_check_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["s3publiccheck", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_ec2_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["ec2monitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_ec2_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["ec2monitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            ...
            self.aws_rds_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["rdsmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_lambda_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["lambdamonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_lambda_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["lambdamonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_es_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["esmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_es_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["esmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_kms_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["kmsmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_kms_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["kmsmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_elb_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["elbmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_elb_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["elbmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_ses_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["sesmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_ses_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["sesmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_dynamodb_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["dynamodbmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_dynamodb_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["dynamodbmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_sns_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["snsmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_sns_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["snsmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_kubernetes_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["kubernetesmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_kubernetes_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["kubernetesmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            ...
            self.aws_apigateway_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["apigatewaymonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_cloudfront_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudfrontmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_cloudfront_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudfrontmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_cloudtrail_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudtrailmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_cloudtrail_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudtrailmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_redshift_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["redshiftmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_redshift_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["redshiftmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            ...
            self.aws_cloudformation_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudformationmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_cloudformation_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudformationmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_sqs_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["sqsmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_sqs_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["sqsmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_cloudwatch_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudwatchmonitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_cloudwatch_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["cloudwatchmonitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_route53_monitoring_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["route53monitoring", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_route53_monitoring_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["route53monitoring", "resTopic"]),
                self.audit_sns_response_topic,
            )
        ...
        if config.get("inventory"):
            ...
            self.aws_inventory_sync_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsinventorysync", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_iam_deepdive_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsiamdeepdive", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_iam_deepdive_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsiamdeepdive", "resTopic"]),
                self.audit_sns_response_topic,
            )
            self.aws_inventory_views_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsinventoryviews", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_inventory_views_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsinventoryviews", "resTopic"]),
                self.audit_sns_response_topic,
            )

        if config.get("monitoring"):
            cfg = config["monitoring"]
            self.aws_remediation_request_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsremediation", "reqTopic"]),
                self.audit_sns_request_topic,
            )
            self.aws_remediation_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awsremediation", "resTopic"]),
                self.audit_sns_response_topic,
            )
            ...
            self.aws_cost_anomaly_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awscostanomaly", "resTopic"]),
                self.audit_sns_response_topic,
            )
            ...
            self.aws_cost_saving_response_topic = self.get_sns_topic(
                self.get_value(cfg, ["awscostsaving", "resTopic"]),
                self.audit_sns_response_topic,
            )
    ...
    def get_value(self, cfg, keys):
        if cfg.get(keys[0]):
            return (
                self.get_value(cfg[keys[0]], keys[1:])
                if len(keys) > 1
                else cfg.get(keys[0])
            )
