...
semgrep_mapping = OntologyMapping(
    ...
    nodes=[
        ...
        # SemgrepSCAFinding is a dependency vulnerability: it is CVE-backed when it
        # has a CVE and advisory-only otherwise. Like GitHubDependabotAlert, its
        # single mapping lives in the CVE semantic mapping (see cves.py), not here.
        # That mapping also carries these SecurityIssue fields (title/severity/
        # status/first_seen) so advisory-only findings keep their normalization.
