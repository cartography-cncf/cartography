...
DEFAULT_OUTPUT_ROOT = Path("generated/rst/modules")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate module schema documentation from the data model.",
    )
    parser.add_argument("modules", nargs="*", help="Module names to generate.")
    parser.add_argument(
        "--all",
        action="store_true",
        help="Generate every module discovered in the data model.",
    )
    parser.add_argument(
        "--preserve-existing",
        action="store_true",
        help="Keep existing manual schema pages instead of replacing them.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=DEFAULT_OUTPUT_ROOT,
        help="Root directory containing module documentation.",
    )
    args = parser.parse_args()
    if args.all == bool(args.modules):
        parser.error("Provide module names or --all, but not both.")

    model = inspect_data_model()
    modules = _model_modules(model) if args.all else args.modules
