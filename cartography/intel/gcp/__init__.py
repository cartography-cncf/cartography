...
def start_gcp_ingestion(
    ...
    # Track org cleanup jobs to run at the very end, and policy-binding sync
    # state across the org loop to guard analysis jobs that depend on fresh
    # APPLIES_TO edges. The projection runs only if at least one org's policy
    # bindings actually synced (`any_policy_bindings_synced`) AND no org's
    # sync failed (`policy_bindings_sync_failed` stays False). An empty orgs
    # list, a sync that never reaches a successful project, or any failure
