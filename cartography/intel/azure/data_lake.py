...
def _get_resource_group_from_id(resource_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Could not parse resource group name from resource ID: {resource_id}")
        return ""
...
def get_datalake_accounts(credentials: Credentials, subscription_id: str) -> list[dict]:
    try:
        ...
        return [
            sa for sa in storage_accounts
            if sa.get('is_hns_enabled')
        ]
...
def get_filesystems_for_account(
    client: StorageManagementClient, account: dict,
) -> list[dict]:
    resource_group_name = _get_resource_group_from_id(account['id'])
    if resource_group_name:
        try:
            return [c.as_dict() for c in client.blob_containers.list(resource_group_name, account['name'])]
        except (ClientAuthenticationError, HttpResponseError) as e:
            logger.warning(f"Failed to get containers for storage account {account['name']}: {str(e)}")
            return []
    return []
...
def transform_datalake_filesystems(filesystems_response: list[dict]) -> list[dict]:
    ...
    for fs in filesystems_response:
        transformed_filesystem = {
            ...
            'has_immutability_policy': fs.get('properties', {}).get('has_immutability_policy'),
...
def load_datalake_filesystems(
    neo4j_session: neo4j.Session, data: list[dict[str, Any]], storage_account_id: str, update_tag: int,
) -> None:
    load(
        neo4j_session,
...
def sync(
    ...
    logger.info(f"Syncing Azure Data Lake File Systems for subscription {subscription_id}.")
    client = StorageManagementClient(credentials.credential, subscription_id)
    ...
    for account in datalake_accounts:
        ...
        load_datalake_filesystems(neo4j_session, transformed_filesystems, account_id, update_tag)
        
        cleanup_params = common_job_parameters.copy()
        cleanup_params["STORAGE_ACCOUNT_ID"] = account_id
        GraphJob.from_node_schema(AzureDataLakeFileSystemSchema(), cleanup_params).run(neo4j_session)