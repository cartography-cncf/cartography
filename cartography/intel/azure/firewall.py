...
def get_firewall_policy_rule_groups(
    ...
    try:
        ...
        rule_groups = list(client.firewall_policy_rule_collection_groups.list(resource_group, policy_name))
...
def transform_firewalls(firewalls: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for fw in firewalls:
        ...
        transformed = {
            ...
            "hub_public_ip_count": len(hub_ip_addresses.get("public_ip_addresses", [])) if hub_ip_addresses.get("public_ip_addresses") else 0,
            
            # Capacity and scaling
            "ip_groups_count": len(fw.get("ip_groups", [])) if fw.get("ip_groups") else 0,
            "autoscale_min_capacity": fw.get("autoscale_configuration", {}).get("min_capacity"),
            "autoscale_max_capacity": fw.get("autoscale_configuration", {}).get("max_capacity"),
            ...
            "ip_configurations": json.dumps(fw.get("ip_configurations")) if fw.get("ip_configurations") else None,
            "management_ip_configuration": json.dumps(fw.get("management_ip_configuration")) if fw.get("management_ip_configuration") else None,
            
            # Rule Collections - security critical (ports, protocols, addresses)
            "network_rule_collections": json.dumps(fw.get("network_rule_collections")) if fw.get("network_rule_collections") else None,
            "application_rule_collections": json.dumps(fw.get("application_rule_collections")) if fw.get("application_rule_collections") else None,
            "nat_rule_collections": json.dumps(fw.get("nat_rule_collections")) if fw.get("nat_rule_collections") else None,
            
            # IP Groups detail - actual IP addresses/ranges used in rules
            "ip_groups_detail": json.dumps(fw.get("_ip_groups_detail")) if fw.get("_ip_groups_detail") else None,
            
            # Counts
            "ip_configuration_count": len(fw.get("ip_configurations", [])),
            "application_rule_collection_count": len(fw.get("application_rule_collections", [])),
            "nat_rule_collection_count": len(fw.get("nat_rule_collections", [])),
            "network_rule_collection_count": len(fw.get("network_rule_collections", [])),
        }
...
def transform_firewall_policies(policies: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for policy in policies:
        ...
        intrusion_config = intrusion_detection.get("configuration", {}) if intrusion_detection else {}
        insights = policy.get("insights", {})
        transport_security = policy.get("transport_security", {})
        cert_authority = transport_security.get("certificate_authority", {}) if transport_security else {}
        ...
        transformed = {
            ...
            "dnsRequireProxyForNetworkRules": dns_settings.get("require_proxy_for_network_rules"),
            "dnsServers": json.dumps(dns_settings.get("servers")) if dns_settings.get("servers") else None,
            
            # SQL Settings
            "sqlAllowSqlRedirect": sql_settings.get("allow_sql_redirect"),
            
            # SNAT Settings - security critical (defines what gets NATted)
            "snatPrivateRanges": json.dumps(snat_settings.get("private_ranges")) if snat_settings.get("private_ranges") else None,
            "snatAutoLearnPrivateRanges": snat_settings.get("auto_learn_private_ranges"),
            ...
            "intrusionDetectionMode": intrusion_detection.get("mode") if intrusion_detection else None,
            "intrusionDetectionProfile": intrusion_detection.get("profile") if intrusion_detection else None,
            
            # Insights
            "insightsIsEnabled": insights.get("is_enabled") if insights else None,
            "insightsRetentionDays": insights.get("retention_days") if insights else None,
            
            # Transport Security
            "transportSecurityCaName": cert_authority.get("name"),
            "transportSecurityKeyVaultSecretId": cert_authority.get("key_vault_secret_id"),
            
            # Threat Intel Whitelist - security critical (bypass rules)
            "threatIntelWhitelistIpAddresses": json.dumps(threat_intel_whitelist.get("ip_addresses")) if threat_intel_whitelist.get("ip_addresses") else None,
            "threatIntelWhitelistFqdns": json.dumps(threat_intel_whitelist.get("fqdns")) if threat_intel_whitelist.get("fqdns") else None,
            
            # Intrusion Detection Configuration - security critical (bypass traffic, signature overrides)
            "intrusionDetectionSignatureOverrides": json.dumps(intrusion_config.get("signature_overrides")) if intrusion_config.get("signature_overrides") else None,
            "intrusionDetectionBypassTraffic": json.dumps(intrusion_config.get("bypass_traffic_settings")) if intrusion_config.get("bypass_traffic_settings") else None,
            "intrusionDetectionPrivateRanges": json.dumps(intrusion_config.get("private_ranges")) if intrusion_config.get("private_ranges") else None,
            
            # Rule Collection Groups - security critical (references to actual rules)
            "ruleCollectionGroups": json.dumps(policy.get("rule_collection_groups")) if policy.get("rule_collection_groups") else None,
            
            # Detailed rule groups with actual security rules (ports, protocols, addresses)
            "ruleGroupsDetail": json.dumps(policy.get("_rule_groups_detail")) if policy.get("_rule_groups_detail") else None,
            
            # Parent/Child relationships
            "childPolicies": json.dumps(policy.get("child_policies")) if policy.get("child_policies") else None,
            "firewalls": json.dumps(policy.get("firewalls")) if policy.get("firewalls") else None,
        }
...
def cleanup(neo4j_session: neo4j.Session, common_job_parameters: dict[str, Any]) -> None:
    ...
    GraphJob.from_node_schema(AzureFirewallSchema(), common_job_parameters).run(neo4j_session)
    
    logger.debug("Running Azure Firewall Policy cleanup job")
    GraphJob.from_node_schema(AzureFirewallPolicySchema(), common_job_parameters).run(neo4j_session)
...
def sync(
    ...
    for policy in policies_data:
        ...
        if policy_id:
            ...
            policy["_rule_groups_detail"] = rule_groups  # Add detailed rules to policy data
    ...
    load_firewall_policies(neo4j_session, transformed_policies, subscription_id, update_tag)
