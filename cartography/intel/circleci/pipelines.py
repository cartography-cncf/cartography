import logging
from typing import Any

import neo4j
import requests

from cartography.client.core.tx import load
from cartography.graph.job import GraphJob
from cartography.intel.circleci.util import paginated_get
from cartography.intel.circleci.util import parse_iso
from cartography.models.circleci.pipeline import CircleCIPipelineSchema
from cartography.util import timeit

logger = logging.getLogger(__name__)

# Pipelines are effectively unbounded; cap pages so a busy project doesn't
# balloon the graph. Raise if deeper pipeline history is ever needed.
_MAX_PIPELINE_PAGES = 5


@timeit
def sync(
    neo4j_session: neo4j.Session,
    api_session: requests.Session,
    common_job_parameters: dict[str, Any],
    project_slug: str,
) -> None:
    raw = get(api_session, common_job_parameters["BASE_URL"], project_slug)
    pipelines = transform(raw, project_slug)
    load_pipelines(
        neo4j_session,
        pipelines,
        common_job_parameters["PROJECT_ID"],
        common_job_parameters["UPDATE_TAG"],
    )
    cleanup(neo4j_session, common_job_parameters)


@timeit
def get(
    api_session: requests.Session,
    base_url: str,
    project_slug: str,
) -> list[dict[str, Any]]:
    return paginated_get(
        api_session,
        f"{base_url}/project/{project_slug}/pipeline",
        max_pages=_MAX_PIPELINE_PAGES,
    )


def transform(
    raw: list[dict[str, Any]],
    project_slug: str,
) -> list[dict[str, Any]]:
    pipelines = []
    for item in raw:
        trigger = item.get("trigger") or {}
        pipelines.append(
            {
                "id": item["id"],
                "number": item.get("number"),
                "state": item.get("state"),
                "project_slug": item.get("project_slug") or project_slug,
                "trigger_type": trigger.get("type"),
                "created_at": parse_iso(item.get("created_at")),
                "updated_at": parse_iso(item.get("updated_at")),
            }
        )
    return pipelines


@timeit
def load_pipelines(
    neo4j_session: neo4j.Session,
    data: list[dict[str, Any]],
    project_id: str,
    update_tag: int,
) -> None:
    load(
        neo4j_session,
        CircleCIPipelineSchema(),
        data,
        lastupdated=update_tag,
        PROJECT_ID=project_id,
    )


@timeit
def cleanup(
    neo4j_session: neo4j.Session,
    common_job_parameters: dict[str, Any],
) -> None:
    GraphJob.from_node_schema(CircleCIPipelineSchema(), common_job_parameters).run(
        neo4j_session,
    )
