...
@timeit
def get_application_data(common_job_parameters: dict[str, Any]) -> list[dict[str, Any]]:
    """
    ...
    """
    account_id = str(common_job_parameters.get("ACCOUNT_ID"))
...
def get_application_installs(
    app_inventory: list[dict[str, Any]], common_job_parameters: dict[str, Any]
) -> list[dict[str, Any]]:
    """
    ...
    logger.info(
        ...
        f"{common_job_parameters.get('ACCOUNT_ID')}",
    )
    ...
    for i, app in enumerate(app_inventory):
        ...
        if "applicationVendor" not in app or "applicationName" not in app:
            logger.error(
                "Application object does not contain applicationVendor or applicationName",
                app,
            )
            continue

        name = app.get("applicationName")
        vendor = app.get("applicationVendor")
        app_installs = get_paginated_results(
            api_url=str(common_job_parameters.get("API_URL")),
            endpoint="/web/api/v2.1/application-management/inventory/endpoints",
            api_token=str(common_job_parameters.get("API_TOKEN")),
            params={
                "accountIds": str(common_job_parameters.get("ACCOUNT_ID")),
                "limit": 1000,
...
def transform_applications(
    ...
    for app in applications_list:
        vendor = app.get("applicationVendor", "").strip()
        name = app.get("applicationName", "").strip()
    ...
    for installed_version in application_installs_list:
        transformed_version = {
            "id": get_application_version_id(
                str(installed_version.get("applicationName")),
                str(installed_version.get("applicationVendor")),
                str(installed_version.get("version")),
            ),
            "application_id": get_application_id(
                str(installed_version.get("applicationName")),
                str(installed_version.get("applicationVendor")),
            ),
            "application_name": installed_version.get("applicationName"),
            "application_vendor": installed_version.get("applicationVendor"),
            ...
            "version": installed_version.get("version"),
        }
...
def sync(
    ...
    account_id_val = common_job_parameters.get("ACCOUNT_ID")
    if account_id_val is None:
        logger.error("ACCOUNT_ID is not set in common_job_parameters")
        return

    account_id = str(account_id_val)
    applications = get_application_data(common_job_parameters)
    application_versions = get_application_installs(applications, common_job_parameters)
