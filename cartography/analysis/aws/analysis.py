...
AWS_EC2_IAM_INSTANCE_PROFILE = AnalysisJob(
    ...
    statements=(
        AnalysisStatement(
            match="MATCH (aa:AWSAccount{id: $AWS_ID})-[:RESOURCE]->(i:AWSEC2Instance)-[:INSTANCE_PROFILE]->(p:AWSInstanceProfile)-[:ASSOCIATED_WITH]->(r:AWSRole)",
            effects=(
                AddRelationship(
                    "i",
...
AWS_LB_CONTAINER_EXPOSURE = AnalysisJob(
    ...
    statements=(
        AnalysisStatement(
            match="MATCH (aa:AWSAccount{id: $AWS_ID})-[:RESOURCE]->(lb:AWSLoadBalancerV2 {scheme: 'internet-facing'})-[:EXPOSE]->(ip:AWSEC2PrivateIp)<-[:PRIVATE_IP_ADDRESS]-(ni:AWSNetworkInterface)<-[:NETWORK_INTERFACE]-(task:AWSECSTask)-[:HAS_CONTAINER]->(c:AWSECSContainer) WHERE ip.public_ip IS NULL",
            effects=(
                AddRelationship(
                    "lb",
                    "EXPOSE",
...
AWS_LB_NACL_DIRECT = AnalysisJob(
    ...
    statements=(
        AnalysisStatement(
            match="MATCH (aa:AWSAccount{id: $AWS_ID})-[:RESOURCE]->(lb:AWSLoadBalancerV2)-[:SUBNET]->(subnet:AWSEC2Subnet)<-[:PART_OF_SUBNET]-(nacl:AWSEC2NetworkAcl)",
            effects=(
                AddRelationship(
                    "nacl",
                    "PROTECTS",
