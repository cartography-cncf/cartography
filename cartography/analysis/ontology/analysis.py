...
DEVICE_AFFECTS_CROWDSTRIKE_FINDING = AnalysisJob(
    ...
    statements=(
        AnalysisStatement(
            match="MATCH (d:Device)-[obs:OBSERVED_AS]->(:CrowdstrikeHost)-[:HAS_VULNERABILITY]->(:CrowdstrikeSpotlightVulnerability)-[:HAS_CVE]->(f:CrowdstrikeFinding) WHERE obs.lastupdated = $UPDATE_TAG AND d.lastupdated = $UPDATE_TAG",
            effects=(
                AddRelationship(
                    "f",
                    "AFFECTS",
...
LOADBALANCER_EXPOSE_CONTAINER = AnalysisJob(
    ...
    statements=(
        AnalysisStatement(
            match="MATCH (lb:LoadBalancer {lastupdated: $UPDATE_TAG})-[:EXPOSE]->(ip:AWSEC2PrivateIp)<-[:PRIVATE_IP_ADDRESS]-(ni:AWSNetworkInterface)<-[:NETWORK_INTERFACE]-(task:AWSECSTask)-[:HAS_CONTAINER]->(c:Container)",
            effects=(
                AddRelationship(
                    "lb",
                    "EXPOSE",
