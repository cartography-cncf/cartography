...
- Cross-cloud detections become trivial: a single `MATCH (:Database)` covers `RDSInstance`, `AWSDynamoDBTable`, `BigQueryDataset`, etc., once those modules opt into the `Database` label.
