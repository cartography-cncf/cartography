...
2. **Plan the merge**:
   - Add the duplicate's `Framework(...)` entries to the keeper's `frameworks=` tuple. Preserve `name`, `short_name`, `scope`, `revision`, `requirement`, and `title` exactly.
...
1. Keep aws_public_rds_instances. Add Framework(short_name="iso27001", requirement="A.13.1.1", title=..., ...). Remove iso27001_public_databases. Update __init__.py.
2. ...
```
...
2. Keeper: `aws_s3_public_buckets`. Action: append `Framework(name="ISO/IEC 27001", short_name="iso27001", requirement="A.13.1.1", title="...")` to its `frameworks=` tuple, delete `storage_public_exposure`, drop the import from `__init__.py`.
