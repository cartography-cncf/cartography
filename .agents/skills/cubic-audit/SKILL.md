...
- **`ignore.files`**: generated and vendored artifacts (e.g. `cartography/data/**` cypher/yaml/json fixtures, `uv.lock`, anything machine-generated) belong in `reviews.ignore.files` so cubic does not spend review budget on them. The current `cubic.yaml` has no `ignore` block; recommend one if review noise on generated files is plausible.
