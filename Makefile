PYTEST_COV_FLAGS := --cov=cartography --cov-branch --cov-report term-missing
...
test_coverage:
	...
	COVERAGE_FILE=.coverage.unit uv run --frozen pytest -vvv $(PYTEST_COV_FLAGS) tests/unit
	COVERAGE_FILE=.coverage.integration uv run --frozen pytest -vvv $(PYTEST_COV_FLAGS) tests/integration
