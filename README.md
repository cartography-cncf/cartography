...
Cartography is a Python tool that pulls infrastructure assets from multiple platforms into a [Neo4j](https://www.neo4j.com) graph database, so you can query the relationships between them.
...
```bash
docker run --publish=7474:7474 --publish=7687:7687 -v data:/data --env=NEO4J_AUTH=none neo4j:5-community
```
...
Cartography will use your configured AWS credentials (`AWS_PROFILE`, environment variables, or instance role). See the [full install guide](https://cartography-cncf.github.io/cartography/install.html) for other platforms.
...
See the [querying tutorial](https://cartography-cncf.github.io/cartography/usage/tutorial.html) and [data schema](https://cartography-cncf.github.io/cartography/usage/schema.html) for more.

### Building applications around Cartography

You can build automation and CSPM (Cloud Security Posture Management) applications on top of the graph. See [applications](https://cartography-cncf.github.io/cartography/usage/applications.html).
